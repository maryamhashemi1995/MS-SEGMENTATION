...
plot_model(model, to_file='model_T2_UNET_challenge2015_Adam_Cstumloss-1.5-1_DSC.png')
# Save the weights
model.save_weights('model_weights_T2_UNET_challenge2015_Adam_Cstumloss-1.5-1_DSC.h5')

# Save the model architecture
with open('model_architecture_T2__UNET_challenge2015_Adam_Cstumloss-1.5-1_DSC.json', 'w') as f:
    f.write(model.to_json())
    
model.save('model_T2__UNET_challenge2015_Adam_Cstumloss-1.5-1_DSC.h5')
...
def parameters(TP_array,TN_array,FP_array,FN_array):
    ...
    ############# Add by Mahsa
    for i in range(0,preds_test.shape[0]-1-4):
...
############## (Add by Mahsa )Sensitivity is correct  (eshtebah type shode bood)
...
############## (Add by Mahsa )Error rate is correct  (eshtebah type shode bood)
