...
#################################################################################            
Y_train1 = np.zeros((1413, IMG_HEIGHT, IMG_WIDTH, 1))
...
X_train1 = np.zeros((1413, IMG_HEIGHT, IMG_WIDTH, IMG_CHANNELS))
...
##########################################
Y_train = np.zeros((1413+1237, IMG_HEIGHT, IMG_WIDTH, 1))
    ...
    for i in range (0,IMG_HEIGHT-1):
        for j in range (0,IMG_WIDTH-1):
            if n<1413:
                ...
                Y_train[n,i,j]=Y_train2[n-1413,i,j]




X_train = np.zeros((1413+1237, IMG_HEIGHT, IMG_WIDTH, IMG_CHANNELS))
    ...
    for i in range (0,IMG_HEIGHT-1):
        for j in range (0,IMG_WIDTH-1):
            if n<1413:
                ...
                X_train[n,i,j]=X_train2[n-1413,i,j]               
...
Y_test1 = np.zeros((80, IMG_HEIGHT, IMG_WIDTH, 1))
...
X_test1 = np.zeros((80, IMG_HEIGHT, IMG_WIDTH, IMG_CHANNELS))
...
Y_test2 = np.zeros((87, IMG_HEIGHT, IMG_WIDTH, 1))
...
X_test2 = np.zeros((87, IMG_HEIGHT, IMG_WIDTH, IMG_CHANNELS))
...
Y_test = np.zeros((87+80, IMG_HEIGHT, IMG_WIDTH, 1))
    ...
    for i in range (0,IMG_HEIGHT-1):
        for j in range (0,IMG_WIDTH-1):
            if n<80:
                ...
                Y_test[n,i,j]=Y_test2[n-80,i,j]




X_test = np.zeros((87+80, IMG_HEIGHT, IMG_WIDTH, IMG_CHANNELS))
...
def parameters(TP_array,TN_array,FP_array,FN_array):
    ...
    return DSC_array,accu_array,sens_array,speci_array,errorrate_array,fnr_array,exfra_array,ppv_array
...
DSC_array,accu_array,sens_array,speci_array,errorrate_array,fnr_array,exfra_array,ppv_array =parameters(TP_array,TN_array,FP_array,FN_array)
...
average_DSC=cul1/(preds_test.shape[0]-1)
average_accuracy=cul2/(preds_test.shape[0]-1)
average_sensitivity=cul3/(preds_test.shape[0]-1)
average_specificity=cul4/(preds_test.shape[0]-1)
average_errorrate=cul5/(preds_test.shape[0]-1)
average_fnr=cul6/(preds_test.shape[0]-1)
average_exfra=cul7/(preds_test.shape[0]-1)
average_ppv=cul8/(preds_test.shape[0]-1)
...
print("average_Errore_Rate",average_errorrate,"average_FNR",average_fnr,"average_Extra_Fraction",average_exfra,"average_PPV",average_ppv)
...
for i in range(preds_train_t.shape[0]):
    for m in range(preds_train_t.shape[1]):
        for n in range(preds_train_t.shape[2]):
            if preds_train[i,m,n,0]>0.3:
                #print(preds_test[i,m,n,0])
                preds_train[i,m,n,0]=1
            else:
                #print(preds_test[i,m,n,0])
                preds_train[i,m,n,0]=0
...
for i in range(preds_train.shape[0]):
    ...
    for m in range(preds_train.shape[1]):
        for n in range(preds_train.shape[2]):
            if preds_train[i,m,n,0]==Y_train[i,m,n,0]==1:
                TP=TP+1
            if preds_train[i,m,n,0]==Y_train[i,m,n,0]==0:
                TN=TN+1
            if preds_train[i,m,n,0]==0 and Y_train[i,m,n,0]==1:
                FN=FN+1
            if preds_train[i,m,n,0]==1 and Y_train[i,m,n,0]==0:
                FP=FP+1
    TP1_array.append(TP) 
    FP1_array.append(FP)
    TN1_array.append(TN)
...
DSC_array,accu_array,sens_array,speci_array,errorrate_array,fnr_array,exfra_array,ppv_array =parameters2(TP1_array,TN1_array,FP1_array,FN1_array)
